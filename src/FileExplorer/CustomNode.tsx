...
export const CustomNode: React.FC<ICustomNode> = (props) => {
  ...
  const NodeIcon = ({name}: { name: string }) => {
    if (showIcon) {
      return (
        <div className='file-explorer__node-icon'>
          {
  ...
  const NodeActions = () => {
    return (
      <div className='file-explorer__node-actions'>
        {
          droppable ? <>
            <span onClick={handleAddNode}><Icon name='addFile'/></span>
            <span onClick={handleAddFolder}><Icon name='addFolder'/></span>
          </> : null
        }
  ...
  return (
    <div
      className={`file-explorer__node ${hover ? 'file-explorer__node--hover' : ''} ${isSelected ? 'file-explorer__node--selected' : ''}`}
    ...
    >
      <div
        className={`file-explorer__node-expand-icon-wrapper ${
          !droppable ? 'disabled' : ''
        }`}
        onClick={handleToggle}
      >
        {droppable && (
          <div className={`file-explorer__node-expand-icon ${
            isOpen ? 'isOpen' : ''
          }`}>
            {switcherIcon ? switcherIcon   : <Icon name='right'/>}
          </div>
        )}
      </div>
      <NodeIcon name={text}/>
